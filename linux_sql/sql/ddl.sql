...
if [ "$#" -ne 5 ]; then
  echo "Error: Illegal number of parameters entered"
...
# Construct the INSERT statement using HEREDOC
insert_stmt=$(cat <<-END
...
if [ $? -ne 0 ]; then
  echo "Error: failed to insert host usage  information the database."
  exit 1
fi

echo "Host usage information was inserted successfully."
